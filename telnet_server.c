...
/* PRIVATE DEFINES */
/*--------------------------------------------------------------------------------------------------------*/
#define SIZE_TOSTRING_BUFFER 128
/*--------------------------------------------------------------------------------------------------------*/
/* PRIVATE VARIABLES */
/*--------------------------------------------------------------------------------------------------------*/
...
static const char ErrorMessage[53] =
        "The Device is already in use...try again later!\r\n";
 ...
 */
static TelnetOpts_t TelnetOptions[] = {
//		{ .option = TELNET_OPT_SUPPRESS_GA, .flags = (0x01 << OPT_FLAG_WILL) },
//		{ .option = TELNET_OPT_ECHO,		.flags = (1 << OPT_FLAG_DO) }
};
...
{
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] Incoming connection requested on Telnet port.\n\r");
#endif
    err_t ret_err;
...
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] A connection was attempted while an active connection is open.\n\r");
#endif
        tcp_accepted(pcb);
        tcp_arg(pcb, NULL);
    ...
    CreateTelnetServer();
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] Initializing telnet server.\n\r");
#endif
    ...
    tcp_accepted(pcb);
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] An incoming connection was accepted.\n\r");
#endif
...
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] Writing the init messages\n\r");
#endif
                           ...
                           err_t err)
{
    ...
    if (arg != NULL)
    {
        ...
        {
#ifdef TELNET_DEBUG
            cli_printf("[Telnet Server] Processing received packet.\n\r");
#endif
    ...
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Could not cast receive args to telnet server struct because they were null.\n\r");
        return ERR_VAL;
#endif
    }
    /* Return okay. */
...
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Sent packet was acknowledged.\n\r");
#endif
...
{
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] Allocating memory for server.\n\r");
#endif
...
TelnetStatus_t InitializeTelnetServer(void)
{
...
#ifdef TELNET_DEBUG
    cli_printf("[Telnet Server] Creating TCP port for Telnet server.\n\r");
#endif
    telnet_pcb = tcp_new();
    ...
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Successfully created Telnet TCP port.\n\r");
#endif
        err_t err;
...
#ifdef TELNET_DEBUG
            cli_printf("[Telnet Server] Now listening for incoming connections on port %i\n\r", TELNET_PORT);
#endif
...
#ifdef TELNET_DEBUG
            cli_printf("[Telnet Server] Can not bind pcb\n\r");
#endif
    ...
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Can not create new TCP port.\n\r");
#endif
 ...
 *
 * @param none
 * @retval uint8_t TRUE = 1 if the telnet server has an active connection.
 */
...
err_t TelnetPoll(void *arg, struct tcp_pcb *tpcb)
{
    ...
    if (server != NULL)
    {
        ...
        {
#ifdef TELNET_DEBUG
            cli_printf("[Telnet Server] Telnet server should be closed.\n\r");
#endif
            TelnetClose();
        }
        ret_err = ERR_OK;
    ...
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Cannot process poll request due to null server structure.\n\r");
#endif
        /* Nothing to be done */
        tcp_abort(tpcb);
        ret_err = ERR_ABRT;
...
void TelnetRecvBufferWrite(char character)
{
    ...
    if (character == 0)
    {
        ...
        cli_printf("[Telnet Server] Ignorning NULL character.\n\r");
#endif
        return;
    }
...
//	}
        ...
        != telnet_server.recvRead)
    {
        ...
        for (int i = 0; i <= telnet_server.recvWrite; ++i) {
            cli_printf("%c", telnet_server.recvBuffer[i]);
        }
        cli_printf("\n\r");
#endif
    } else
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Could not store new character because buffer was full.\n\r");
#endif
    }
...
void TelnetWrite(const char character)
{
    ...
    {
#ifdef TELNET_DEBUG
        cli_printf("[Telnet Server] Telnet buffer is full!\n\r");
#endif
...
void TelnetProcessWill(char option)
{
    unsigned long ulIdx;
#ifdef TELNET_CHAR_DEBUG
    cli_printf("[Telnet Server] Processing WILL command with option: %c/0x%02X\n\r", option, option);
#endif
...
void TelnetProcessWont(char option)
{
    unsigned long ulIdx;
#ifdef TELNET_CHAR_DEBUG
    cli_printf("[Telnet Server] Processing WONT command with option: %c/0x%02X\n\r", option, option);
#endif
    /* Loop through the known options. */
    for(ulIdx = 0; ulIdx < (sizeof(TelnetOptions) / sizeof(TelnetOptions[0]));
        ulIdx++)
...
void TelnetProcessDo(char option)
{
    unsigned long ulIdx;
#ifdef TELNET_CHAR_DEBUG
    cli_printf("[Telnet Server] Processing DO command with option: %c/0x%02X\n\r", option, option);
#endif
    /* Loop through the known options. */
    for(ulIdx = 0; ulIdx < (sizeof(TelnetOptions) / sizeof(TelnetOptions[0]));
        ulIdx++)
...
void TelnetProcessDont(char option)
{
    unsigned long ulIdx;
#ifdef TELNET_CHAR_DEBUG
    cli_printf("[Telnet Server] Processing DONT command with option: %c/0x%02X\n\r", option, option);
#endif
    /* Loop through the known options. */
    for(ulIdx = 0; ulIdx < (sizeof(TelnetOptions) / sizeof(TelnetOptions[0]));
        ulIdx++)
...
{
#ifdef TELNET_CHAR_DEBUG
    cli_printf("[Telnet Server] Processing Character: %c/0x%02X\n\r", character, character);
#endif
